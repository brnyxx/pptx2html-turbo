...
The Rust core preserves ordered slide transition/timing XML and approximately executes a bounded interaction-driven subset: cut/fade transitions and click/with-previous/after-previous appear, disappear, or fade effects on resolved slide shapes. It never autoplays or loops; unsupported timing remains typed fallback metadata and unsupported targets stay statically visible. The public `Slide::timing`, timing model types, `FeatureFamily::Animation`, and `FallbackKind::TimingMetadata` additions require exhaustive matches and slide struct literals to migrate on the next semver-major line; package versions intentionally remain unchanged here.
