...
Adding `actions` to the public `Shape` and `TextRun` structs, adding the public typed action enums, and adding `FallbackKind::ActionMetadata` require external exhaustive matches and struct literals to migrate in the next semver-major line. The manifest version is intentionally unchanged in this task; consumers should use `..Default::default()` where appropriate and treat `actions` as authoritative.
