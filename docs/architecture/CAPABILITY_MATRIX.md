...
| Charts | `approximate` | `rendered` | `approximate` | Chunk 3 | One namespace-aware classifier covers every classic family and ChartEx, rejects combinations and incompatible axis/series or variant semantics, and admits only the existing bounded direct subset. Every rejection preserves raw chart XML and emits one typed fallback diagnostic; a deterministic safe internal preview is preferred, otherwise a visible placeholder is rendered. No exact chart-fidelity claim is made. |
