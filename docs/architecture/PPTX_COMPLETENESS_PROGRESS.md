...
| 18 | Bounded audio/video delivered | Shape-owned official internal PCM WAV and AVC MP4 assets render approximately with native controls and user-gesture playback; unsafe or unsupported cases remain typed poster/placeholder fallbacks. Browser evidence is recorded locally; native PowerPoint equivalence remains `[교차검증 필요]`. |
...
| 20 | Delivered | Accepted | Namespace-aware chart classification covers classic families and ChartEx; only compatible bounded cases render directly, while raw XML, deterministic preview/placeholder fallback, and one typed diagnostic prevent silent loss. |
