...
PDF_CHECKSUM_PATTERN = re.compile(
    rb"(/DocChecksum\s*/)([0-9A-Fa-f]+)",
)
XMP_DATE_TAGS = ("CreateDate", "ModifyDate", "MetadataDate")
...
def normalize_pdf_bytes(value: bytes) -> bytes:
    result = PDF_DATE_PATTERN.sub(
        lambda match: match.group(1) + b"0" * len(match.group(2)) + match.group(3),
        value,
    )
    result = PDF_ID_PATTERN.sub(
        lambda match: (
            match.group(1)
    ...
    if len(result) != len(value):
        raise PdfConformanceError("PDF normalization changed byte length")
    return result
