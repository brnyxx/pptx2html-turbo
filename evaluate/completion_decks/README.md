...
The table deck pairs one package-defined custom style with a loadable missing-definition fallback. The fallback preserves the official built-in `Medium Style 2 - Accent 1` GUID in `a:tableStyleId` while deliberately omitting that style from `ppt/tableStyles.xml`, which lets later work verify ID preservation and fallback diagnostics without inventing an invalid GUID.
