...
Use the complete pinned-provenance shape in `metadata.example.json`; the root cross-linked batch shape is in `manifest.example.json`. Source, metadata, and slide hashes are emitted by the capture tooling and must not be hand-authored.
