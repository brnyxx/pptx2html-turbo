    ...
    def test_picture_bullet_deck_has_independent_size_and_failure_stimuli(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with zipfile.ZipFile(output / "picture-bullets.pptx") as archive:
                ...
                self.assertIsNone(
                    paragraphs["Missing reference"]
                    .find("a:pPr/a:buBlip/a:blip", NS)
                    .get(f"{{{NS['r']}}}embed")
                )
                self.assertEqual(
                    paragraphs["Wrong kind"]
                    .find("a:pPr/a:buBlip/a:blip", NS)
                    .get(f"{{{NS['r']}}}embed"),
                    "rIdWrongKind",
                )
                self.assertEqual(
                    paragraphs["Unsupported SVG"]
                    .find("a:pPr/a:buBlip/a:blip", NS)
                    .get(f"{{{NS['r']}}}embed"),
                    "rIdSvg",
                )
                self.assertEqual(
                    paragraphs["Linked external"]
                    .find("a:pPr/a:buBlip/a:blip", NS)
                    .get(f"{{{NS['r']}}}link"),
                    "rIdLinked",
                )
                ...
                self.assertTrue(
                    archive.read("ppt/media/bullet.png").startswith(b"\x89PNG")
                )
