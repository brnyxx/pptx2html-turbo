...
pub(super) const RUNTIME: &str = r#"<script>(()=>{const go=(e)=>{const n=e.target.closest('[data-action]');if(!n)return;e.pptx2htmlActionHandled=true;const a=n.dataset.action;if(a==='media'){const m=n.closest('.shape')?.querySelector('audio,video');if(m){e.preventDefault();m.play().catch(()=>{})}return}if(!['slide','next','previous','first','last'].includes(a))return;e.preventDefault();const s=[...document.querySelectorAll('.slide')],c=n.closest('.slide'),i=s.indexOf(c);let t=a==='slide'?document.getElementById('slide-'+n.dataset.slideTarget):a==='first'?s[0]:a==='last'?s.at(-1):s[Math.max(0,Math.min(s.length-1,i+(a==='next'?1:-1)))];if(t)location.hash=t.id};document.addEventListener('click',go);document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches('[role=button][data-action]')){e.preventDefault();e.target.click()}})})();</script>"#;

pub(super) const CSS: &str = ".run[data-action],.run[data-hover-action],.shape-action-surface~.text-body .run[href],.shape-action-surface~table .run[href],.shape-action-surface~.shape .run[href],.shape-action-surface~.shape>.shape-action-surface{pointer-events:auto}.shape-action-surface{position:absolute;inset:0;z-index:1;border:0;background:transparent;cursor:pointer}.media-action-surface{inset:8px 8px auto auto;width:36px;height:36px;z-index:3;border-radius:50%;background:rgba(0,0,0,.55)}.shape-media{position:relative;z-index:2;width:100%;height:100%;pointer-events:auto}.shape-audio{object-fit:contain}.shape-video{object-fit:contain}.media-fallback{width:100%;height:100%;object-fit:contain}.media-placeholder{display:flex;align-items:center;justify-content:center;background:#eee;color:#555}.shape-action-surface~.text-body,.shape-action-surface~table{position:relative}.shape-action-surface~.text-body,.shape-action-surface~table,.shape-action-surface~.shape{z-index:2;pointer-events:none}";
