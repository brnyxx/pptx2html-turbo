...
pub(super) fn frame_path(w: f64, h: f64, adj: &HashMap<String, f64>) -> String {
    let adjustment = adj.get("adj1").or_else(|| adj.get("adj"));
    let t = w.min(h) * adjustment.copied().unwrap_or(12500.0) / 100_000.0;
    format!(
...
"#;
pub(super) fn gear_path(w: f64, h: f64, teeth: u32, adj: &HashMap<String, f64>) -> String {
    match (teeth, adj.is_empty()) {
        (6, true) => scale_normalized_path(GEAR_6_DEFAULT_NORMALIZED_PATH, w, h),
        (9, true) => scale_normalized_path(GEAR_9_DEFAULT_NORMALIZED_PATH, w, h),
        _ => {
            let (cx, cy) = (w / 2.0, h / 2.0);
            let default_adj1 = if teeth == 6 { 15_000.0 } else { 10_000.0 };
            let default_adj2 = if teeth == 6 { 3_526.0 } else { 1_763.0 };
            let short_side = w.min(h);
            let outer = short_side * 0.5;
            let inner =
                outer - short_side * adj.get("adj1").copied().unwrap_or(default_adj1) / 100_000.0;
            let rotation =
                (adj.get("adj2").copied().unwrap_or(default_adj2) / 60_000.0).to_radians();
            let t = teeth * 2;
