...
pub(super) fn snip2_same_rect_path(w: f64, h: f64, adj: &HashMap<String, f64>) -> String {
    ...
    let top = scaled(
        ss,
        finite(adj.get("adj1").copied(), 16_667.0).clamp(0.0, 50_000.0),
    );
    let bottom = scaled(
        ss,
        finite(adj.get("adj2").copied(), 0.0).clamp(0.0, 50_000.0),
    );
    format!(
...
pub(super) fn snip2_diag_rect_path(w: f64, h: f64, adj: &HashMap<String, f64>) -> String {
    ...
    let left = scaled(
        ss,
        finite(adj.get("adj1").copied(), 0.0).clamp(0.0, 50_000.0),
    );
    let right = scaled(
        ss,
        finite(adj.get("adj2").copied(), 16_667.0).clamp(0.0, 50_000.0),
    );
    format!(
...
pub(super) fn round1_rect_path(w: f64, h: f64, adj: &HashMap<String, f64>) -> String {
    let r = scaled(
        w.min(h),
        finite(adj.get("adj").copied(), 16_667.0).clamp(0.0, 50_000.0),
    );
    format!(
